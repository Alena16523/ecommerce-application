...
public class Main {
    ...
    public static void main(String[] args)  {
        ...
        Cart cart = new Cart(customer); //to save products in the cart
        ...
        while(true){
            ...
            switch (menuSelection){
                ...
                case 4:
                    ...
                    break;
                case 5:
                    Map<Product,Integer> map = new HashMap<>(); //initializing map from the product class
                    cart.setProductMap(map); //assigning map to the cart
                    while(true){
                        System.out.println("Which product you want to add to your cart: enter product ID. For exit product selection Type : exit");
                        for(Product product: StaticConstants.PRODUCT_LIST){
                            try {
                                System.out.println(
                        ...
                        try {
                            Product product = findProductById(productId); //exception
                            if (!putItemToCartIfStockAvailable(cart, product)) { //true false
                                System.out.println("Stock is insufficient. Please try again");
                                continue;
                            }
                            System.out.println("Product was added to the cart: "+product.getName());
                        } catch (Exception e) {
                    ...
                    if(!(cart.getProductMap().isEmpty() || cart.getProductMap()==null)){
                        ...
                        if (!discountId.equals("no")) {
                            try {
                                ...
                                if (discount.decideDiscountIsApplicableToCart(cart)) { //true or false
    ...
    private static Discount findDiscountById(String discountId) throws Exception {
        ...
        throw new Exception("Discount can not be applied, not available.");
    }
    ...
    private static boolean putItemToCartIfStockAvailable(Cart cart, Product product) {
        ...
        if(cartCount !=null && product.getRemainingStock() >= cartCount+count){
            ...
            return true; //milk is in the cart already and buying more milk
